...
const USER_SEEDS: UserSeed[] = [
  {
    ...
    email:
      process.env.SEED_ACADEMIC_COORDINATOR_EMAIL ?? "coordinator@planify.edu",
...
async function ensureRole(
  ...
  if (existing) {
    ...
    if (!updated) {
      throw new Error(
        `Role ${roleSeed.type} could not be reloaded after update.`
      );
    }
...
async function removeDangerousPublicPermissions(
  ...
  for (const permission of publicRole.permissions) {
    if (
      actionsToRemove.has(
        permission.action as (typeof ACADEMIC_COORDINATOR_ACTIONS)[number]
      )
    ) {
      await strapi.db.query("plugin::users-permissions.permission").delete({
        where: { id: permission.id },
...
export default {
  ...
  async bootstrap({ strapi }: { strapi: Core.Strapi }) {
    ...
    for (const userSeed of USER_SEEDS) {
      ...
      if (!role?.id) {
        throw new Error(
          `Role ${userSeed.roleType} was not found for user ${userSeed.email}.`
        );
      }
